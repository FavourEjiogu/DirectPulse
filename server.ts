...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST') {
        if (req.url === '/api/triage') {
            ...
            if (!symptoms) {
                res.writeHead(400);
                res.end(JSON.stringify({ error: 'symptoms is required' }));
                return;
            }
        ...
        if (req.url === '/api/refine') {
            const body = await getBody(req);
            const { currentInput } = body;
            if (!currentInput || currentInput.length < 8) {
                res.writeHead(200);
                res.end(JSON.stringify({ suggestion: null }));
                return;
            }
            ...
            try {
                ...
                if (text === "Looks detailed enough." || !text) {
                    res.writeHead(200);
                    res.end(JSON.stringify({ suggestion: null }));
                } else {
                    res.writeHead(200);
